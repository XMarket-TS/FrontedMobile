                        ...
                        GestureDetector(
                          onTap: () {
                            ...
                            _bank.text.isNotEmpty
                                ? _update.bank = _bank.text
                                : _update.bank = _card.bank;
                            _cardNumber.text.isNotEmpty
                                ? _update.cardNumber =
                                    int.parse(_cardNumber.text)
                                : _update.cardNumber = _card.cardNumber;
                            _expirationYear.text.isNotEmpty
                                ? _update.expirationYear =
                                    int.parse(_expirationYear.text)
                                : _update.expirationYear = _card.expirationYear;
                            _expirationMonth.text.isNotEmpty
                                ? _update.expirationMonth =
                                    int.parse(_expirationMonth.text)
                                : _update.expirationMonth =
                                    _card.expirationMonth;
                            _cvc.text.isNotEmpty
                                ? _update.cvc = int.parse(_cvc.text)
                                : _update.cvc = _card.cvc;
                            _update.creationDate = _card.creationDate;
                            _update.status = _card.status;
                            print("prueba list update");
                            print(_update.bank);
                            BlocProvider.of<NavigationBloc>(context)
                                .add(UpdateCardEvent(_update, _card.userId));
                          },
                          child: Container(
                        ...
                        GestureDetector(
                          onTap: () {
                            BlocProvider.of<NavigationBloc>(context).add(
                                DeleteCardEvent(_card.cardId, _card.userId));
                          },
                          child: Container(
                            alignment: Alignment.center,
                            width: size.width * 0.3,
